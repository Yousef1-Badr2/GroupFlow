      ...
      <div className="flex-1 overflow-y-auto space-y-6 pb-20">
        {projectPolls.length === 0 ? (
          ...
          projectPolls.map((poll) => {
            ...
            return (
              <div key={poll.id} className="bg-white dark:bg-[#1E1E1E] rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-5">
                <div className="flex justify-between items-start mb-4">
                  ...
                  {poll.closed ? (
                    <span className="text-xs font-bold uppercase tracking-wider bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400 px-2 py-1 rounded-full whitespace-nowrap">
                      Closed
                    </span>
                      ...
                      <div key={index} className="relative">
                        <button
                          ...
                          className={`w-full flex justify-between items-center p-3 rounded-xl border transition-all relative z-10 ${
                            isMyVote 
                              ...
                              : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-[#1E1E1E] hover:border-primary-400 dark:hover:border-primary-800'
                          }`}
                        >
                        ...
                        <div 
                          className={`absolute top-0 left-0 h-full rounded-xl transition-all duration-500 ease-out ${
                            isMyVote ? 'bg-primary-100 dark:bg-primary-950/40' : 'bg-slate-100 dark:bg-slate-800/50'
                          }`}
              ...
              {options.map((option, index) => (
                <div key={index} className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 p-3 bg-slate-50 dark:bg-[#121212] border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-600"
                  />
                  {options.length > 2 && (
                    <button 
                      type="button" 
